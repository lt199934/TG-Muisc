(function($){
	// Settings
	var isShowNotification = false,//显示通知
		isInitMarquee = true,//初始化字幕
		shuffleArray = [],
		shuffleIndex,
		autoClearTimer,
		autoShowTimer,
	...
	for (var i = 0; i < playlist.length; i++){
		var item = playlist[i];
		$('#playlist').append('<li class="lib" style="overflow:hidden;"><input type="hidden" value="'+item.index+'"><strong style="margin-left: 5px;">'+item.title+'</strong><span id="del" style="margin-left: 40px;display:none;" class="glyphicon glyphicon-trash">删除</span><span style="float: right;" class="artist">'+item.artist+'</span></li>');
		if (item.mp3 == "") {
	...
	$("#QPlayer .ssBtn").on('click', function(){
		var mA = $("#QPlayer");
		console.log($('.ssBtn .adf').hasClass('on'))
